document.addEventListener("DOMContentLoaded", async function() {
    ...
    let accounts = [];
    const contractAddress = '0x0899FD82e498EB6792487A86d1cd022a3a0BD9BD';
            const contractABI = [
                {
                    "inputs": [
                        {
    ...
    itemForm.addEventListener("submit", async function(event) {
        ...
        const price = document.getElementById("price").value;//formData.get("price").value;
        ...
        try{
            ...
            items.forEach(item => {
                ...
                if((item.owner.toUpperCase() != accounts[0].toUpperCase()) && item.available == true) {
                    const card = document.createElement('div');
                    card.classList.add('card');
                    //const itemPriceInEther = w3.utils.fromWei(item.price.toString(), 'ether');
                    const cardContent = `
                        <h2>${item.title}</h2>
                        <p>${item.description}</p>
                        <p>Price: ${w3.utils.fromWei(item.price.toString(), 'ether')} ETH</p>
